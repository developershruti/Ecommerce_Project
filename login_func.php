...
if ($username=='guest' && $password=='123456') {
	...
	$_SESSION['arr_error_msgs'] = $arr_error_msgs;
 	if (count($arr_error_msgs)==0) {  
  	$sql = "select * from ngo_users where u_email = '$username' and u_status='Active' ";
  	$result = db_query($sql);
			...
			$sql_log = "insert into ngo_login_log set log_userid='$u_id', log_ip='$ip', log_date=now()    ";
 			...
 			if ($_SESSION['sess_back']!='') {
				...
				exit;	
 				...
 				header("location:userpanel/account-overview.php");
				exit;	
 			}
			
		} else {
 		$arr_error_msgs[] =  "Invalid User ID/password try again!" ;
		$_SESSION['arr_error_msgs'] = $arr_error_msgs;
			header("Location: login.php");
	        exit;
		}
 	} else { 
		$arr_error_msgs[] =  "Invalid User ID/password  or Inactive account!" ;
		$_SESSION['arr_error_msgs'] = $arr_error_msgs;
		header("Location: login.php");
		exit;
}
