...
if (is_post_back()) {
  ...
  if (count($arr_error_msgs) == 0) {
    ...
    if ($s_id != '') {
      $sql = "update ngo_services set s_title = '$s_title', s_scate_id = '$s_scate_id' " . $update_photo . $update_subcate .", s_description = '$s_description' , s_updated_by ='" . $_SESSION['sess_admin_userid'] . "' where s_id = '$s_id'";

      db_query($sql);
      $arr_success_msgs[] = "Services details updated successfully!!";
    } else {
      $sql = "insert into ngo_services set s_title = '$s_title', s_scate_id = '$s_scate_id' "  . $update_photo . $update_subcate . ", s_created_by = '" . $_SESSION['sess_admin_userid'] . "', s_status = 'Active' , s_description = '$s_description'  ";
      db_query($sql);
