...
if (is_post_back()) {
  ...
  //if ($s_id =='') { $arr_error_msgs[] =  "User ID  does not exist!";}
  ...
  if (count($arr_error_msgs) == 0) {
    ...
    if ($s_id != '') {
        $sql = "update ngo_services set s_title = '$s_title', s_scate_id = '$s_scate_id' " . $update_photo . ", s_description = '$s_description', s_updated_by ='" . $_SESSION['sess_admin_userid'] . "' where s_id = '$s_id'";
	  
      db_query($sql);
      $arr_success_msgs[] = "Services details updated successfully!!";
    } else {
      $sql = "insert into ngo_services set s_title = '$s_title', s_scate_id = '$s_scate_id' " . $update_photo . ", s_created_by = '" . $_SESSION['sess_admin_userid'] . "', s_status = 'Active' , s_description = '$s_description' ";
      db_query($sql);
    ...
    if ($sms_send == '1') {
      ...
      $sql = "update ngo_services set u_password = '$password' where u_id in ($str_u_ids)";
      db_query($sql);
    } else if (isset($_REQUEST['Activate']) || isset($_REQUEST['Activate_x'])) {
      $sql = "update ngo_services set s_status = 'Active' where s_id in ($str_u_ids)";
      db_query($sql);
    } else if (isset($_REQUEST['Deactivate']) || isset($_REQUEST['Deactivate_x'])) {
      $sql = "update ngo_services set s_status = 'Inactive' where s_id in ($str_u_ids)";
      db_query($sql);
    }
...
<body>
 ...
 <?php
 		...
 		echo make_dropdown($sql_scate, 's_scate_id', $s_scate_id, 'class="form-control"  style="width: 100%;color:#000;" alt="select" emsg="Select Service Category"', 'Select Service Category');
 ?>
                                            <?php
 											
                      	/// $sql_scatess = "SELECT scate_id, concat(scate_id, ' - ', scate_name) FROM ngo_service_cate WHERE scate_status='Active' ORDER BY scate_id ";
 						/// make_dropdown($sql_scatess, 's_scate_id,', $s_scate_id, 'class="form-control"  style="width: 100%;color:#000;" alt="select" emsg="Select Service Category"', 'Select Service Category');
                                            ?>
                                          </div>
                                        </div>
