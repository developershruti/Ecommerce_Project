...
if (is_post_back()) {
	...
	if ($u_login_old != $u_login) {
		$u_login_count = db_scalar("select count(*) from ngo_users where u_login = '$u_login'");
		if ($u_login_count > 0) {
	...
	// if ($u_login2_old != $u_login2) {
	// 	$u_login2_count = db_scalar("select count(*) from ngo_users where u_login2 = '$u_login2'");
	// 	if ($u_login2_count > 0) {
	...
	if (count($arr_error_msgs) == 0) {
		...
		/*if (is_array($permission_check)) {
			$u_permission = implode(',', $permission_check);
		}*/
		/*
	
	if($_POST['u_picture_del']!='') {
		$sql_edit_part .= ", u_picture = '' ";
	}
	if($_FILES['u_picture']['name']!='') {
		$u_picture_name = rawurlencode($_FILES['u_picture']['name']).'-'.md5(uniqid(rand(), true)).'.'.file_ext($_FILES['u_picture']['name']);
		copy($_FILES['u_picture']['tmp_name'], UP_FILES_FS_PATH.'/'.$u_picture_name);
		$sql_edit_part .= ", u_picture = '$u_picture_name' ";
	}*/
		//u_login = '$u_login',
	 
		$sql = "update ngo_users set u_password = '$u_password', u_email = '$u_email', u_gender = '$u_gender' ,  u_fname = '$u_fname', u_lname = '$u_lname',  u_address = '$u_address', u_city = '$u_city', u_state = '$u_state', u_postalcode = '$u_postalcode' , u_country = '$u_country',   u_country_code = '$u_country_code', u_mobile='$u_mobile' ,u_description='$u_description', u_acc_type='Customer'  ,u_date=ADDDATE(now(),INTERVAL 750 MINUTE),u_datetime=ADDDATE(now(),INTERVAL 750 MINUTE)  $sql_edit_part where u_id='$_SESSION[sess_uid]'";
		db_query($sql);
...
$u_userid = $_SESSION['sess_uid']; /// $_REQUEST['u_userid']
if ($u_userid != '') {
	$sql = "select * from ngo_users where u_id = '$u_userid'";
	$result = db_query($sql);
	if ($line_raw = mysqli_fetch_array($result)) {
		$line = ms_form_value($line_raw);
		@extract($line);
		$permission_check = explode(",", $u_permission);
	}
}
?>
<!DOCTYPE html>
...
<body>
	...
	<script>
			...
			if (document.getElementById('address_confirm').checked) {
				document.getElementById('u_address').value = document.getElementById('u_curr_address').value;
			} else {
